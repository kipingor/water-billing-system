...
var mergeConfig = (method2, url, data, config) => ({
  url,
  method: method2,
  ...config,
...
var createValidator = (callback, initialData = {}) => {
  ...
  const createValidator2 = () => debounce_default(() => {
    callback({
    ...
    return {
      ...
      onBefore: () => {
        const beforeValidationResult = (config.onBeforeValidation ?? ((previous, next) => {
          return !isEqual_default(previous, next);
...
function dist_default(Alpine) {
  Alpine.magic("form", (el) => (method2, url, inputs, config = {}) => {
    ...
    const createForm = () => ({
      ...
      validate(name) {
        if (typeof name === "undefined") {
          ...
          name = resolveName(name);
          validator.validate(name, get_default(form.data(), name));
        }
        return form;
      },
      validating: false,
    ...
    syncWithDom(el, resolveMethod(method2), resolveUrl(url), form);
    return form;
  });
}
var syncWithDom = (el, method2, url, form) => {
...
var addProcessingListener = (el, form) => el.addEventListener("submit", () => form.processing = true);
export {
  client,
